...
for ((i=0; i < $NUM_WORKERS; i++)); do
  ...
  ssh ${SSH_USER}@${WORKER_HOSTNAMES[i]} sudo iptables -D FORWARD -j REJECT --reject-with icmp-host-prohibited
  ssh ${SSH_USER}@${WORKER_HOSTNAMES[i]} sudo iptables -D INPUT -j REJECT --reject-with icmp-host-prohibited
...
for ((i=0; i < $NUM_PROXYS; i++)); do
  ...
  ssh ${SSH_USER}@${PROXY_HOSTNAMES[i]} sudo iptables -D FORWARD -j REJECT --reject-with icmp-host-prohibited
  ssh ${SSH_USER}@${PROXY_HOSTNAMES[i]} sudo iptables -D INPUT -j REJECT --reject-with icmp-host-prohibited
done
