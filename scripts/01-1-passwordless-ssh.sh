...
for ((i=0; i < $NUM_WORKERS; i++)); do
  ...
  scp ~/.ssh/id_rsa.pub ${SSH_USER}@${WORKER_HOSTNAMES[i]}:~/id_rsa.pub
  ssh ${SSH_USER}@${WORKER_HOSTNAMES[i]} 'mkdir -p ~/.ssh; cat ~/id_rsa.pub | tee -a ~/.ssh/authorized_keys'
done
...
for ((i=0; i < $NUM_PROXYS; i++)); do
  ...
  # Copy over key (Will prompt for password)
  scp ~/.ssh/id_rsa.pub ${SSH_USER}@${PROXY_HOSTNAMES[i]}:~/id_rsa.pub
  ssh ${SSH_USER}@${PROXY_HOSTNAMES[i]} 'mkdir -p ~/.ssh; cat ~/id_rsa.pub | tee -a ~/.ssh/authorized_keys'
done
